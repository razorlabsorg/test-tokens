module razor_test_tokens::razor_token {
    ...
    const ASSET_SYMBOL: vector<u8> = b"RZR";
    const ASSET_NAME: vector<u8> = b"Razor Token";
    const DECIMALS: u8 = 8;
    ...
    fun owner(): address acquires Ownership {
        let owner = borrow_global<Ownership>(razor_token_address()).owner;
        return owner
    }

    #[view]
    public fun razor_token_address(): address {
        object::create_object_address(&@razor_test_tokens, ASSET_SYMBOL)
    }

    #[view]
    public fun metadata(): Object<Metadata> {
        object::address_to_object(razor_token_address())
    }
    ...
    public entry fun mint(minter: &signer, to: address, amount: u64) acquires Management, Ownership, State {
        ...
        let management = borrow_global<Management>(razor_token_address());
    ...
    public entry fun set_pause(admin: &signer, paused: bool) acquires Ownership, State {
        let ownership = borrow_global<Ownership>(razor_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
        let state = borrow_global_mut<State>(razor_token_address());
    ...
    public entry fun denylist(admin: &signer, account: address) acquires Management, Ownership, State {
        ...
        let freeze_ref = &borrow_global<Management>(razor_token_address()).transfer_ref;
    ...
    public entry fun transfer_ownership(admin: &signer, new_owner: address) acquires Ownership, State {
        assert_not_paused();
        let ownership = borrow_global_mut<Ownership>(razor_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
    ...
    fun assert_not_paused() acquires State {
        let state = borrow_global<State>(razor_token_address());
