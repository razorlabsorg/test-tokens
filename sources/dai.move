module razor_test_tokens::dai_token {
    ...
    fun owner(): address acquires Ownership {
        let owner = borrow_global<Ownership>(dai_token_address()).owner;
        return owner
    }

    #[view]
    public fun dai_token_address(): address {
        object::create_object_address(&@razor_test_tokens, ASSET_SYMBOL)
    }

    #[view]
    public fun metadata(): Object<Metadata> {
        object::address_to_object(dai_token_address())
    }
    ...
    fun init_module(admin: &signer) {
        ...
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            ...
            utf8(ASSET_SYMBOL),
            DECIMALS,
            utf8(b"https://raw.githubusercontent.com/razorlabsorg/chainlist/refs/heads/main/chain/aptos/asset/DAI.png"),
            utf8(b"https://makerdao.com"),
        );
    ...
    public entry fun mint(minter: &signer, to: address, amount: u64) acquires Management, Ownership, State {
        ...
        let management = borrow_global<Management>(dai_token_address());
    ...
    public entry fun set_pause(admin: &signer, paused: bool) acquires Ownership, State {
        let ownership = borrow_global<Ownership>(dai_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
        let state = borrow_global_mut<State>(dai_token_address());
    ...
    public entry fun denylist(admin: &signer, account: address) acquires Management, Ownership, State {
        ...
        let freeze_ref = &borrow_global<Management>(dai_token_address()).transfer_ref;
    ...
    public entry fun transfer_ownership(admin: &signer, new_owner: address) acquires Ownership, State {
        assert_not_paused();
        let ownership = borrow_global_mut<Ownership>(dai_token_address());
        assert!(signer::address_of(admin) == ownership.owner, ERROR_UNAUTHORIZED);
    ...
    fun assert_not_paused() acquires State {
        let state = borrow_global<State>(dai_token_address());
